...
app.delete('/devices/:ip', (req, res) => {
    ...
    if (deviceIndex !== -1) {
        const removedDevice = devicePool.splice(deviceIndex, 1)[0];
        res.json({message: `Device ${deviceIp} has been removed.`, device: removedDevice});
    } else {
...
const port = 3000;
app.listen(port, () => {
    console.log(`Server is running on port ${port}\ndocs: http://localhost:${port}/api-docs\nmanage page: http://localhost:${port}`);
});
